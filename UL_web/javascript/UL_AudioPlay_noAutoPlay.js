...
function updateWaveWidgetValue(widgets, id, url, prompt, wavesurfer) {
  let widget = widgets.filter(w => w.name == 'UL_Audio_Preview_noAutoPlay')[0]
  // 手动更新widget值
...
app.registerExtension({
  name: 'UL.Audio_Preview_noAutoPlay',
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeType.comfyClass == 'UL_Audio_Preview_noAutoPlay') {
      let that = this
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        const widget = {
          type: 'div',
          name: 'UL_Audio_Preview_noAutoPlay',
  ...
  async loadedGraphNode(node, app) {
    if (node.type === 'UL_Audio_Preview_noAutoPlay') {
      let widget = node.widgets.filter(w => w.name == 'UL_Audio_Preview_noAutoPlay')[0]
