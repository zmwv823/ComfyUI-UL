...
class CompressionModel(ABC, nn.Module):
    ...
    def get_pretrained(
        ...
        if name in ['dac_44khz', 'dac_24khz']:
            ...
            facebook_encodec_32khz_model_path = os.path.join(folder_paths.models_dir, 'audio_checkpoints\ExtraModels\models--facebook--encodec_32khz')
            if not os.access(os.path.join(facebook_encodec_32khz_model_path, "model.safetensors"), os.F_OK):
                facebook_encodec_32khz_model_path = 'facebook/encodec_32khz'
            print("\033[93mLoad facebook encodec_32khz model from(从此位置加载facebook encodec_32khz模型)：\n    ", facebook_encodec_32khz_model_path, "\n脚本位置第121行：\n    ", current_file_path, "\033[0m")
            hf_model = HFEncodecModel.from_pretrained(facebook_encodec_32khz_model_path)
