...
class TextRecognizer(object):
    def __init__(self, args, predictor):
        ...
        self.chars = self.get_char_dict(os.path.join(base, 'custom_nodes\ComfyUI-UL\AnyText\ocr_weights\ppocr_keys_v1.txt'))
...
def main():
    rec_model_dir = os.path.join(base, 'custom_nodes\ComfyUI-UL\AnyText\ocr_weights\ppv3_rec.pth')
    predictor = create_predictor(rec_model_dir)
    args = edict()
    args.rec_image_shape = "3, 48, 320"
    args.rec_char_dict_path = os.path.join(base, 'custom_nodes\ComfyUI-UL\AnyText\ocr_weights\ppocr_keys_v1.txt')
